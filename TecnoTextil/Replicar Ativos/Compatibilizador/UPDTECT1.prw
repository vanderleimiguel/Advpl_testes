...
/*/
//--------------------------------------------------------------------
User Function UPDTECT1( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDTECT1" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDTECT1" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SN1'																	, .T. }, ; //X3_ARQUIVO
	{ '98'																	, .T. }, ; //X3_ORDEM
	{ 'N1_XREPATV'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Replic Ativo'														, .T. }, ; //X3_TITULO
	{ 'Replic Ativo'														, .T. }, ; //X3_TITSPA
	{ 'Replic Ativo'														, .T. }, ; //X3_TITENG
	{ 'Replic Ativo'														, .T. }, ; //X3_DESCRIC
	{ 'Replic Ativo'														, .T. }, ; //X3_DESCSPA
	{ 'Replic Ativo'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Verifica se ativo ja foi replicado' )

PutSX1Help( "PN1_XREPATV", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "N1_XREPATV" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDTECT1" ) ) ) ;
