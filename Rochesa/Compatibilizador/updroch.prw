...
/*/
//--------------------------------------------------------------------
User Function UPDROCH( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDROCH" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDROCH" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Cod.Val.Decl'														, .T. }, ; //X3_TITULO
	{ 'Cod.Val.Decl'														, .T. }, ; //X3_TITSPA
	{ 'Cod.Val.Decl'														, .T. }, ; //X3_TITENG
	{ 'Cod Valores Declaratorios'											, .T. }, ; //X3_DESCRIC
	{ 'Cod Valores Declaratorios'											, .T. }, ; //X3_DESCSPA
	{ 'Cod Valores Declaratorios'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
aAdd( aSX3, { ;
	...
	{ 7																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Cod. Reflexo'														, .T. }, ; //X3_TITULO
	{ 'Cod. Reflexo'														, .T. }, ; //X3_TITSPA
	{ 'Cod. Reflexo'														, .T. }, ; //X3_TITENG
	{ 'Cod. Reflexo Lancamento'												, .T. }, ; //X3_DESCRIC
	{ 'Cod. Reflexo Lancamento'												, .T. }, ; //X3_DESCSPA
	{ 'Cod. Reflexo Lancamento'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
aAdd( aSXA, { ;
	'SF4'																	, ; //XA_ALIAS
	'4'																		, ; //XA_ORDEM
	'Relacionamento TES x Cod.Val.D'										, ; //XA_DESCRIC
	'Relacionamento TES x Cod.Val.D'										, ; //XA_DESCSPA
	'Relacionamento TES x Cod.Val.D'										, ; //XA_DESCENG
	''																		, ; //XA_AGRUP
	''																		, ; //XA_TIPO
	'U'																		} ) //XA_PROPRI
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'DB'																	, ; //XB_COLUNA
	'Cod Val Declaratorio'													, ; //XB_DESCRI
	'Cod Val Declaratorio'													, ; //XB_DESCSPA
	'Cod Val Declaratorio'													, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'CDY'																	} ) //XB_CONTEM
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'01'																	, ; //XB_COLUNA
	'Cod.ajuste + Data In'													, ; //XB_DESCRI
	'Cod.ajuste + Fecha I'													, ; //XB_DESCSPA
	'Adjust Cd + Start Da'													, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	''																		} ) //XB_CONTEM
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'01'																	, ; //XB_COLUNA
	'Cod.Ajuste'															, ; //XB_DESCRI
	'Cod.Ajuste'															, ; //XB_DESCSPA
	'Adjust Cd'																, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'CDY_CODAJU'															} ) //XB_CONTEM
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'DB'																	, ; //XB_COLUNA
	'Cod Reflexo'															, ; //XB_DESCRI
	'Cod Reflexo'															, ; //XB_DESCSPA
	'Cod Reflexo'															, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'CE0'																	} ) //XB_CONTEM
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'DB'																	, ; //XB_COLUNA
	'Cod Observacao Fisca'													, ; //XB_DESCRI
	'Cod Observacao Fisca'													, ; //XB_DESCSPA
	'Cod Observacao Fisca'													, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'CCE'																	} ) //XB_CONTEM
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Data de Inclusao' )

PutSX1Help( "PF3K_DINCLU", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F3K_DINCLU" )

//
// Helps Tabela SF4
//
aHlpPor := {}
aAdd( aHlpPor, 'Fórmula que contém mensagem a ser im-' )
aAdd( aHlpPor, 'pressa na coluna "Observaçöes" dos' )
aAdd( aHlpPor, 'Livros Fiscais Registro de Entrada e' )
aAdd( aHlpPor, 'Saídas.' )

PutSX1Help( "PF4_FORMULA", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F4_FORMULA" )

aHlpPor := {}
aAdd( aHlpPor, 'Este campo deve ser atribuído com o' )
aAdd( aHlpPor, 'conteúdo ?2=Não? nas situações onde a' )
aAdd( aHlpPor, 'gravação das informações do ipi na' )
aAdd( aHlpPor, 'observação devam ser desconsideradas' )
aAdd( aHlpPor, 'mesmo utilizando o parâmetro MV_IPINOBS' )
aAdd( aHlpPor, 'com seu conteúdo igual a ?SS?' )
...
aAdd( aHlpPor, '5.2 EFD ICMS IPI' )

PutSX1Help( "PF4_XCODAJU", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F4_XCODAJU" )

aHlpPor := {}
aAdd( aHlpPor, 'Código de Reflexo do Lançamento. Ao' )
aAdd( aHlpPor, 'preencher este campo com um Código de' )
aAdd( aHlpPor, 'Reflexo, os valores de Base, Alíquota e' )
aAdd( aHlpPor, 'Valor deste lançamento serão baseados' )
aAdd( aHlpPor, 'nesta amarração.' )

PutSX1Help( "PF4_XCODREF", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F4_XCODREF" )

aHlpPor := {}
aAdd( aHlpPor, 'Código da observação a ser utilizada no' )
aAdd( aHlpPor, 'documento fiscal. Estas informações' )
aAdd( aHlpPor, 'equivalem a observações que são' )
aAdd( aHlpPor, 'lançadasna coluna Observações dos Livros' )
aAdd( aHlpPor, 'Fiscais.' )

PutSX1Help( "PF4_XIFCOMP", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F4_XIFCOMP" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDROCH" ) ) ) ;
